...
**v1 範圍（已確認停損點，見第11節）**：本輪只承諾做到 Phase 0~3——
lexer/parser、含宏程序（變量/GOTO/IF/WHILE/G65/M98）、G32與G90/G92/G94
單一型固定循環、matplotlib 靜態圖與動畫。複合型固定循環（G70~G76）、
刀尖半徑補償、網頁互動 viewer **不在本輪承諾範圍**，先在第7、8、13節
把設計想清楚避免日後大改，但不在這次排期內實作，待 v1 出貨、且有實際
NC 檔案可驗證後再評估是否/如何繼續。
...
**v1 停損點：Phase 0~3（已確認）**。Phase 0~3 是本輪承諾要做完、要出貨
的範圍——含用戶宏程序（變量/控制流/G65/M98）與單一型固定循環，做完後
先停下來，用實際能跑、能畫圖的東西評估。**Phase 4 以後（複合型固定循環
G70~G76、刀尖半徑補償、網頁viewer、stock removal）先不承諾排期**，等
v1 出來、且有實際 NC 檔案可驗證後再決定是否/如何繼續。第7、8、13節等
針對 Phase 4+ 的設計已經先想清楚，是為了避免 Phase 0~3 的資料結構挖坑
（例如 `programmed_end`、`tool_comp` 介入方式），但實作本身不在 v1 範圍。
...
- **Phase 4**（待評估）複合型固定循環，依第7節建議順序逐一實作
  （G71→G70→G72→G73→G76→G74/G75）。G76 的 `P(m)(r)(a)` 位數編碼解析
  需獨立單元測試。
