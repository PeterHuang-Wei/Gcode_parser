...
- **Phase 5** 網頁 viewer（JSON匯出 + Canvas 顯示完整路徑 + 連續播放動畫，
  只求連續不求時間精確，見第9節）。matplotlib 的靜態圖與動畫（`viz_
  matplotlib.py`）可以更早（例如 Phase 0 就先有靜態圖）逐步加，不必等到
  Phase 5 才第一次出圖；Phase 5 主要是把同等能力搬到網頁上。
