...
18. **命名式巨集變量別名（例如 `#_OFST`）與警告可見性**（使用者實測
    真實 NC 檔案發現的兩個問題，同一輪處理）：
    - **`#_OFST` 這種底線開頭的命名變量別名，兩份手冊摘錄都沒有記載**
      （可能是特定機台/後處理器的系統變量別名慣例），但原本
      `parse_var_index` 只接受數字或中括號表達式，遇到命名別名會直接
      `ParseError`，**中止整支程式的解析**（比第14點的「未知G代碼」更
      嚴重，因為那個階段還沒開始執行，一次性挋垮整個模擬）。新增
      `NamedVarRef`（`ast_nodes.py`）與 `parse_hash_value`
      （`expression.py`，`_parse_primary` 的 `#` 分支與
      `parser.py._parse_nc_word_value` 的 `#` 分支共用）：只在**讀取**
      場合容忍命名別名（賦值目標 `#_OFST=...` 仍維持原本嚴格報錯，因為
      沒有實際變量可寫入），一律當成空值（`EMPTY`）並透過
      `warnings.warn` 印出「這是什麼」，而不是整支程式解析失敗。
    - **警告訊息本身可能沒被看到，才是「完全沒顯示路徑卻不知道為什麼」
      的根本原因**：`warnings.warn` 預設只印到 stderr，在沒有終端機視
      窗的桌面 GUI 環境下可能完全不可見。`gui.py` 新增一個「Warnings
      from last run」唯讀文字面板，`run()` 用
      `warnings.catch_warnings(record=True)` 包住整次模擬、執行完後把
      擷取到的警告列出來，並在狀態列明確標註「toolpath is EMPTY」；
      `cli.py` 也比照做法，把擷取到的警告明確印到 stderr、並在
      moves 數為 0 時額外印一行提示去看上面的警告。
    - **桌面 GUI 的開檔對話框加入 `.cnc`/`.txt` 副檔名**（原本只有
      `.nc`），因為使用者的檔案不一定用 `.nc` 命名。
